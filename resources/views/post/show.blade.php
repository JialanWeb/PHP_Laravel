        ...
        <div class="col-md-12">
            <div class="card blau">
                ...
                <div class="card-body">
                    ...
                    <em class="d-block text-secondary"><b>Verknüpfte Kategorien</b>(klicken zum Entfernen)</em>
                    @foreach($post->tags AS $tag)
                        <a href="/post/{{ $post->id }}/tag/{{$tag->id}}/detach" class="badge bg-{{$tag->style}} text-decoration-none">
                        {{$tag->name}}
                        </a>
                    @endforeach
                    </div>
                    ...
                    <div class="alert alert-secondary">
                    <em class="d-block text-secondary"><b>Verfügbare Kategorien</b>(klicken zum einfügen)</em>
                    @foreach($availableTags AS $tag)
                        <a href="/post/{{ $post->id }}/tag/{{$tag->id}}/attach" class="badge bg-{{$tag->style}} text-decoration-none">
                        {{$tag->name}}
                        </a>
                    @endforeach
                    </div>
